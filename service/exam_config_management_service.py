...
def get_candidate_by_eid_cid(candidate_id, exam_config_id):
    candidate = Candidate.query.filter_by(id=candidate_id, exam_config_id=exam_config_id).first()
    return candidate


def candidate_login(email, exam_config_id, password_hash):
    candidate = Candidate.query.filter_by(email=email, exam_config_id=exam_config_id,
                                          password_hash=password_hash).first()
    dt = datetime.utcnow()
    if candidate is not None:
        if candidate.end_time is not None:
            return "C"
        exam_config = ExamConfig.query.filter_by(id=candidate.exam_config_id, exam_owner_id=candidate.exam_owner_id).first()
