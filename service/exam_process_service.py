...
def prepare_candidate_exam(exam_config_id, candidate_id):
    ...
    if count == 0:
        ...
        session_candidate_app.query(Candidate).filter_by(id=candidate_id, exam_config_id=exam_config_id).update({'start_time': dt})
...
def get_exam_questions(candidate_id, exam_config_id, page, per_page):
    ...
    result = session_candidate_exam_app.execute(
        'SELECT ce.id, ce.exam_questions_id, ce.candidate_id, ce.exam_config_id, ce.is_choice1_selected, ce.is_choice2_selected, ce.is_choice3_selected, ce.is_choice4_selected, ce.is_choice5_selected,\
        eq.question, eq.choice1, eq.choice2, eq.choice3, eq.choice4, eq.choice5, eq.question_type, eq.negative_marks, eq.positive_marks\
        FROM candidate_exam ce inner join exam_questions eq on ce.exam_questions_id = eq.id \
        WHERE ce.candidate_id=:val1 and ce.exam_config_id=:val2 ORDER BY ce.id LIMIT :offset, :row_count ',
...
def update_candidate_exam(cand_exam):
    session_candidate_exam_app.query(CandidateExam).filter_by(id=cand_exam.id,
        ...
        {'is_choice1_selected': cand_exam.is_choice1_selected, 'is_choice2_selected': cand_exam.is_choice2_selected, 'is_choice3_selected': cand_exam.is_choice3_selected,
...
def candidate_exam_finish(exam_config_id, candidate_id):
    dt = datetime.utcnow()
    session_candidate_app.query(Candidate).filter_by(id=candidate_id, exam_config_id=exam_config_id).update({'end_time': dt})
    session_candidate_app.commit()
